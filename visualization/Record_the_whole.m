% MATLAB 绘图脚本：根据集合中 UAV 成员来确定颜色（动画版），并录制视频
% 自定义函数：读取 JSON 文件
...
% 计算短半轴长度 b，使得椭圆面积固定为 900*pi（这里固定900）
fixed_area = 900;
...
videoFileName = fullfile(save_path, ['video_' timeStamp '.mp4']);
writerObj = VideoWriter(videoFileName, 'MPEG-4');
writerObj.FrameRate = 10;   % 根据需要调整帧率
...
for step = 1:num_steps
    ...
    if step > 1
        ...
        delete(findobj(gca, 'Type', 'scatter', 'Marker', 'o')); % Robots（若有）
