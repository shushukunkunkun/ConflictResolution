...
class QMixNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.two_hyper_layers:
            self.hyper_w1 = nn.Sequential(
                ...
                nn.Linear(args.hyper_hidden_dim,
                          args.n_agents * args.qmix_hidden_dim))
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Sequential(
                nn.Linear(args.state_shape, args.hyper_hidden_dim), nn.ReLU(),
                nn.Linear(args.hyper_hidden_dim, args.qmix_hidden_dim))
        else:
            self.hyper_w1 = nn.Linear(args.state_shape,
                                      args.n_agents * args.qmix_hidden_dim)
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Linear(args.state_shape,
                                      args.qmix_hidden_dim * 1)
        ...
        self.hyper_b2 = nn.Sequential(
            nn.Linear(args.state_shape, args.qmix_hidden_dim), nn.ReLU(),
    ...
    def forward(self, q_values, states
        ...
        q_values = q_values.view(-1, 1, self.args.n_agents).float(
        ...
        w1 = w1.view(-1, self.args.n_agents,
                     self.args.qmix_hidden_dim)  # (1920, 5, 32)
